...
| **Binary size** | ~300KB | ~200KB | ~2.6MB (static) | ~150KB | ~400KB | ~350KB | ~500KB | ~1.5MB |
| **Startup time** | Instant | Instant | Instant | Instant | Instant | Instant | Instant | ~0.5s |
| **Learning curve** | Easy | Medium | Medium | Medium | Easy | Medium | Easy | Hard |
| **Notation** | Music notation | Concise DSL | S-expressions | Algebraic | Method calls | S-expressions | Method calls | DSL + combinators |
...
;; Concurrent voices
(spawn (make-melody-voice (scale c4 'dorian) mf eighth) "melody")
...
- Full GNU Guile 3.0 with modules, macros, GOOPS
- Same API as s7-midi (shared prelude)
- Async scheduler with spawn/run/poll/stop
- Static linking on macOS (only system libs required)
- Access to Guile's extensive library ecosystem
- Better debugging and error messages than s7

**Weaknesses:**

- Requires Guile 3.0 installed (not embedded like s7)
- Larger binary size due to static linking
- Heavier runtime than s7

**Choose if:** You want a full-featured Scheme with modules and ecosystem, or already use Guile.
...
| Memory usage | ~3MB | ~2MB | ~8MB | ~2MB | ~4MB | ~3MB | ~5MB | ~10MB |
| Test count | 3 | 82 | 33 | 14 | 26 | 33 | 22 | 149 |
...
| **Sequence System** | Tick-based events | Yes (64 seq, 256 events) | No | No | No | No | No | No |
| **Async Launch** | Yes (always) | Yes (seq-play&) | Yes (spawn/run) | No (planned) | Yes (spawn/run) | Yes (spawn/run) | Yes (spawn/run) | Yes (spawn/run) |
| **Host Concurrency** | libuv thread | libuv thread | libuv + thunks | Synchronous | libuv + coroutines | libuv + thunks | libuv + generators | Native threads |
